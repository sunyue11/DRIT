...
class IDCM_NN(nn.Module):
    ...
    def forward(self, img, text):
        ...
        view1_feature = normalize(view1_feature.cpu().detach(), norm = 'l2')
        view2_feature = self.text_net(text.float())
        view2_feature = normalize(view2_feature.cpu().detach(), norm = 'l2')
        view1_feature = torch.from_numpy(view1_feature).cuda(0)
        view2_feature = torch.from_numpy(view2_feature).cuda(0)
        view1_feature = self.linearLayer(view1_feature.float())
        view2_feature = self.linearLayer(view2_feature.float())
