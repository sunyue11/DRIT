...
import torch
from model_2 import VSE, order_sim
...
def i2t(images, captions, npts=None, measure='cosine', return_ranks=False):
    ...
    for index in range(npts):
        ...
        if measure == 'order':
            ...
            d = numpy.dot(im.cpu().detach(), captions.cpu().detach().T).flatten()
...
def t2i(images, captions, npts=None, measure='cosine', return_ranks=False):
    ...
    if npts is None:
        npts = int(images.shape[0] / 5)
    ims = numpy.array([numpy.array(images[i].cpu().detach()) for i in range(0, len(images), 5)])
    ...
    for index in range(npts):
        ...
        if measure == 'order':
            ...
            d = numpy.dot(queries.cpu().detach(), ims.T)
