...
class Dis_content(nn.Module):
    def __init__(self):
      ...
      model = []
      model += [LeakyReLUConv2d(256, 256, kernel_size=1, stride=1, padding=0, norm='Instance')]
      model += [LeakyReLUConv2d(256, 256, kernel_size=1, stride=1, padding=0, norm='Instance')]
      model += [LeakyReLUConv2d(256, 256, kernel_size=1, stride=1, padding=0, norm='Instance')]
      model += [LeakyReLUConv2d(256, 256, kernel_size=1, stride=1, padding=0)]
      model += [nn.Conv2d(256, 1, kernel_size=1, stride=1, padding=0)]
      self.model = nn.Sequential(*model)#这样就把多个网络集合到一起，变成多层网络结构
...
class E_content(nn.Module):
  def __init__(self, input_dim_a, input_dim_b):
    ...
    encA_c = []
    tch = 64  #之前是64
    encA_c += [LeakyReLUConv2d(input_dim_a, tch, kernel_size=7, stride=1, padding=3)]#[batch,64,32,32]
    for i in range(1, 3):
      encA_c += [ReLUINSConv2d(tch, tch * 2, kernel_size=3, stride=2, padding=1)]#[64,32,32]->[128,16,16]
      tch *= 2                                                                   #[128,16,16]->[256,8,8]
    for i in range(0, 3):
      encA_c += [INSResBlock(tch, tch)]#[batch,tch*2,原来//4,原来//4]

    encB_c = []
    tch = 64
    encB_c += [LeakyReLUConv2d(input_dim_b, tch, kernel_size=7, stride=1, padding=3)]#【batch,tch,300,300]
    for i in range(1, 3):
      encB_c += [ReLUINSConv2d(tch, tch * 2, kernel_size=3, stride=2, padding=1)]#[150,150]->[75,75]
      tch *= 2
    for i in range(0, 3):
      encB_c += [INSResBlock(tch, tch)]

    enc_share = []#要保证映射到统一公共子空间，所以Encoder最后一层参数共享
    for i in range(0, 1):
...
class E_attr_concat(nn.Module):
  ...
  def forward(self, xa, xb):
    x_conv_A = self.conv_A(xa)#[20,256,1,1]
    conv_flat_A = x_conv_A.view(xa.size(0), -1)#先卷积 后reshape，再通过线性层  #[20,256]
    output_A = self.fc_A(conv_flat_A)#[20,8]
...
####################################################################
class G_a(nn.Module):
  def __init__(self,opts,output_dim_a,output_dim_b):
    ...
    decA1 = []
    for i in range(0, 3):
      decA1 += [INSResBlock(tch, tch)] #[38,38]
    tch = tch + self.nz
    decA2 = ReLUINSConvTranspose2d(tch, tch//2, kernel_size=3, stride=2, padding=1, output_padding=1)#[152,152]
    tch = tch//2
    tch = tch + self.nz
    decA3 = ReLUINSConvTranspose2d(tch, tch//2, kernel_size=3, stride=2, padding=1, output_padding=1)#[64,64]????
    tch = tch//2
    tch = tch + self.nz
    ...
    decB1 = []
    for i in range(0, 3):
      decB1 += [INSResBlock(tch, tch)]   #[38,38]
    tch = tch + self.nz
    decB2 = ReLUINSConvTranspose2d(tch, tch//2, kernel_size=3, stride=2, padding=1, output_padding=1)#[152,152]
    tch = tch//2
    tch = tch + self.nz
    decB3 = ReLUINSConvTranspose2d(tch, tch//2, kernel_size=3, stride=2, padding=1, output_padding=1)#[304,304]
    tch = tch//2
    tch = tch + self.nz
  ...
  def forward_a(self,a):
      ...
      latent_dim = a.shape[-1]#8
      image_size = 8
      a = a.view(batch_size, latent_dim, 1, 1)
      a = a.repeat(1, 1, image_size, image_size)#在特定的维度进行平铺
      a = self.grl.apply(a)
      out1 = self.decA1(a)
      out2 = self.decA2(out1)
      out3 = self.decA3(out2)
      out4 = self.decA4(out3)
      return out4

  def forward_b(self,a):
      batch_size = a.shape[0]
      latent_dim = a.shape[-1]#8
      image_size = 8
      a = a.view(batch_size, latent_dim, 1, 1)
...
class G_concat(nn.Module):
  def __init__(self, output_dim_a, output_dim_b, nz):
    ...
    self.dec_share = nn.Sequential(*dec_share)#因为映射到同一空间，所以要G的第一层共享参数
  ...
  def forward_a(self, x, z,lin=False):
    ...
    if lin:
        z=out4.view(z.size(0),-1)
        out4=self.decA5(z)
    return out4

  def forward_b(self, x, z,lin=False):
    out0 = self.dec_share(x)
    z_img = z.view(z.size(0), z.size(1), 1, 1).expand(z.size(0), z.size(1), x.size(2), x.size(3))
    x_and_z = torch.cat([out0,  z_img], 1)
